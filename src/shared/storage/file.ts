...
export class FileTokenStore implements TokenStore {
  ...
  private readonly encryptor: Encryptor | null = null;
  ...
  private async loadAsync() {
    if (!this.persistPath) {
      logger.debug("file_token_store", {
        message: "No persistPath, skipping load",
      });
      return;
    }
    try {
  ...
  private async saveAsync() {
    ...
    try {
      ...
      try {
        ...
        /* chmod best-effort, ignore errors */
      }
      logger.debug("file_token_store", {
        message: "File saved",
  ...
  async storeRsMapping(
    ...
    const result = await this.memory.storeRsMapping(
      rsAccess,
      provider,
      rsRefresh
    );
  ...
  async getByRsAccess(rsAccess: string) {
    return await this.memory.getByRsAccess(rsAccess);
  }

  async getByRsRefresh(rsRefresh: string) {
    return await this.memory.getByRsRefresh(rsRefresh);
  }
  ...
  async saveTransaction(txnId: string, txn: Transaction, ttlSeconds?: number) {
    return await this.memory.saveTransaction(txnId, txn, ttlSeconds);
  }

  async getTransaction(txnId: string) {
    return await this.memory.getTransaction(txnId);
  }

  async deleteTransaction(txnId: string) {
    return await this.memory.deleteTransaction(txnId);
  }

  async saveCode(code: string, txnId: string, ttlSeconds?: number) {
    return await this.memory.saveCode(code, txnId, ttlSeconds);
  }
